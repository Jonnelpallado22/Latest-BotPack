...
const ADMIN = 'Jonnel_Pallado';
const FB_LINK = 'https://www.facebook.com/profile.php?id=61556125234351';
...
.then(response => {
    ...
    return api.sendMessage("Hey There!", event.threadID, () => api.sendMessage({ body: `✅ Bot's connected in this Group ${threadName}`, attachment: fs.createReadStream(gifPath)}, threadID));
})
.catch(error => {
    console.error(error);
});
    ...
    try {
      ...
      (typeof threadData.customJoin == "undefined") ? msg = `🌟 Hello and Welcome new member {name} to this group {threadName}.\n\n→ {type} are the the number {soThanhVien}${suffix} member of this group.\n─────────────────\n[ {time} - {thu} ]` : msg = threadData.customJoin;
